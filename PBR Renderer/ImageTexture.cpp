...
HRESULT ImageTexture::init(Graphics& graphics, ID3D11ShaderResourceView** view, DXGI_FORMAT format, std::string imgFile) {
	...
	if(SUCCEEDED(result)) {
		...
		result = graphics.getDevice()->CreateShaderResourceView(texture, &resourceDesc, view);
...
HRESULT ImageTexture::initDDS(Graphics& graphics, ID3D11Texture2D** resource, ID3D11ShaderResourceView** view, std::string ddsFile) {
	DDS_HEADER header;
	...
	result = graphics.getDevice()->CreateTexture2D(&textureDesc, resourceData, resource);
	...
	if(SUCCEEDED(result)) {
		...
		result = graphics.getDevice()->CreateShaderResourceView(*resource, &resourceDesc, view);
	}
...
HRESULT ImageTexture::initMaterial(Graphics& graphics, ID3D11ShaderResourceView** view, std::string material) {
	// texture packing: textures can have 1, 2, or 4 channels
	// 1: albedo + x (4 channels srgb)
	// 2: normal + height (4 channels)
	// 3: roughness + metallic + ao + emissive (4 channels)

	DDS_HEADER header;
	DDS_HEADER_DXT10 header10;
	unsigned char* data;

	HRESULT result = readDDSFile(material, header, header10, data);
	if(FAILED(result)) return result;
